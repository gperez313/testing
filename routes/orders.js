  ...
  router.patch('/:id', authRequired, async (req, res) => {
    ...
    try {
      ...
      await sessionDb.withTransaction(async () => {
        ...
        if (updates.driverScannedItems !== undefined) {
          order.driverScannedItems = updates.driverScannedItems;
        }
        ...
        if (requestedStatus === 'ASSIGNED') {
          ...
          // If it's a return, we don't apply credits yet. Admin must APPROVE it.
          // But we do update the verified counts if provided by the driver.
          if (isReturnOnly) {
          ...
          // Admin approval step for returns
          if (order.status !== 'DELIVERED' && order.status !== 'PICKED_UP') {
             const e = new Error('Order must be delivered/picked up before approval.');
             e.code = 'INVALID_APPROVAL';
             throw e;
          }

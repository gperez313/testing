      ...
      if (type === 'checkout.session.completed') {
        ...
        if (orderId) {
          await sessionDb.withTransaction(async () => {
            ...
            // 🚀 Real-Time Driver Orchestration
            // Notify drivers about a new order ready for assignment
            if (req.app.locals.io) {
              // Notify all drivers
              req.app.locals.io.to('drivers').emit('order:new', {
              ...
              req.app.locals.io.to(`user:${order.customerId}`).emit('order:authorized', {
                orderId: order.orderId,
                status: 'AUTHORIZED'
              });
            }

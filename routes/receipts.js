...
export const approveReceiptJobHandler = async (req, res) => {
  ...
  try {
    ...
    // Step 1: Loyalty & Rewards Integration
    // Award points/credits to the user who created the capture or the customer of the order
    try {
      const approvedTotal = itemsToApprove.reduce((sum, item) => sum + (item.totalPrice || 0), 0);
      await awardLoyaltyForReceipt({
        userId: capture.createdByUserId,
        orderId: capture.orderId || parseJob?.orderId,
        totalAmount: approvedTotal,
        itemCount: itemsToApprove.length,
        session
      });
    } catch (loyaltyError) {
      console.error('Failed to award loyalty rewards:', loyaltyError);
      // We don't abort the transaction for loyalty failures as the receipt data is more critical

...
interface ManagementReceiptProps {
  fmtTime: (_iso?: string) => string;
  stores: StoreRecord[];
  activeStoreId: string;
  setActiveStoreId: (_id: string) => void;
  refreshStores: () => Promise<void>;
  isLoadingStores: boolean;
  storeError: string | null;
  setStoreError: (_error: string | null) => void;
}
...
const ManagementReceipt: React.FC<ManagementReceiptProps> = ({
  ...
  const [_expandedJob, _setExpandedJob] = useState<string | null>(null);
  ...
  const updateReceiptApprovalItem = useCallback(
    (itemId: string, updater: (_item: ReceiptApprovalDraftItem & { id: string }) => ReceiptApprovalDraftItem & { id: string }) => {
  ...
  const handleCommit = useCallback(async () => {
    ...
    try {
      ...
      const _summarizedErrors = summarizeApprovalErrors(backendErrors);
      ...
      if (approvalData?.error || appliedCount < 1) {
        ...
        const failureSummary = _summarizedErrors ? `${backendReason} ${_summarizedErrors}` : backendReason;
  ...
  const handleReceiptCaptured = useCallback((_captureId: string) => {
  ...
  const handleApproveReceipt = useCallback(
    ...
    [addToast, finalStoreMode, finalStoreDraft, receiptApprovalItems, receiptApprovalIdempotencyKey, receiptApprovalNotes, ignorePriceLocks, refreshStores, fetchProducts, setActiveStoreId, updateStoreDraft]
  );

  const handleRejectParseJob = useCallback(async (jobId: string) => {
    setIsProcessing(true);
    try {
      const _data = await receiptApiClient.rejectJob(jobId);

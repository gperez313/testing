...
interface ManagementDashboardProps {
  ...
  products: Product[];
  auditModel: string;
  setAuditModel: (_model: string) => void;
  auditModels: string[];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/5 rounded-xl p-6 border border-white/10">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              {isOpsSummaryLoading ? 'Analyzing…' : 'Ops Summary'}
            </button>
            <button
              onClick={runAudit}
        ...
        {(aiInsights || opsSummary) && (
          <div className="mt-4 p-4 bg-ninpo-midnight/60 rounded-lg border border-white/5">
            {opsSummary && (
              <div className="mb-4 pb-4 border-b border-white/5">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-ninpo-lime mb-2">Operations Summary</h4>
                <p className="text-xs text-slate-300 leading-relaxed">{opsSummary}</p>
              </div>
            )}
            {aiInsights && (
              <div>
                <h4 className="text-[10px] font-black uppercase tracking-widest text-ninpo-lime mb-2">AI Audit Report</h4>
                <pre className="text-xs text-slate-300 leading-relaxed whitespace-pre-wrap">{aiInsights}</pre>
              </div>
            )}
          </div>
        )}

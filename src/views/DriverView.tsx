...
const DriverView: React.FC<DriverViewProps> = ({ currentUser, orders, updateOrder }) => {
  ...
  const _formatConfidence = (value?: number) => {
  ...
  const normalizeUpc = useCallback((value: string) => String(value || '').replace(/\D/g, '').trim(), []);

  const extractUpcCandidates = useCallback((item: any) => {
  ...
  const _startVerification = async (order: Order) => {
    setActiveOrder(order);
    resetPhotoState();
  ...
  const _takeDeliveryPhoto = async () => {
    const dataUrl = captureFromVideo();
    if (!dataUrl) return;
    setCapturedPhoto(dataUrl);
  };

  const _takeReturnPhoto = () => {
    const dataUrl = captureFromVideo();
    if (!dataUrl) return;
  ...
  const _decrementUpc = (upc: string, reason: QuantityChangeReason = 'manual_decrement') => {
  ...
  const addVerificationScan = async (upcRaw: string, _source: 'scanner' | 'manual' = 'manual') => {
    const upc = String(upcRaw || '').replace(/\s+/g, '').trim();
    if (!upc) return;
  ...
  const _explainCaptureIssue = async () => {
  ...
  const _completeDelivery = () => {
    ...
    const _expectedReturnCount = getExpectedReturnCount(activeOrder);
  ...
  const _completionTitle = !paymentCaptured && !isReturnOnly
  ...
  const _handleCapturePaymentClick = () => {
    if (showScanSummary) {

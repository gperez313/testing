...
export const leaveRoom = (room: string) => {
  if (!socket) return;
  socket.emit('leave', room); // Server needs to handle 'leave' if we want explicit leave
};

export const onDriverLocation = (callback: (data: { driverId: string; coords: { lat: number; lng: number } }) => void) => {
  if (!socket) return () => {};

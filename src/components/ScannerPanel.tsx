...
const ToastContext = React.createContext<{ addToast: (_msg: string, _opts?: { type?: 'success' | 'error' | 'info' }) => void } | null>(null);
import {
  X,
  ScanLine,
  Camera,
...
export interface ScannerPanelProps {
  mode?: ScannerMode;
  onScan?: (_upc: string) => void;
  onCooldown?: (_upc: string, _reason: 'cooldown' | 'duplicate') => void;
  title?: string;
  subtitle?: string;
  beepEnabled?: boolean;
  cooldownMs?: number;
  onPhotoCaptured?: (_photoDataUrl: string, _mime: string) => void;
  onReceiptParsed?: (_items: ParsedReceiptItem[], _frame?: string) => void;
  onModeChange?: (_mode: ScannerMode) => void;
...
const ScannerPanel = forwardRef<any, ScannerPanelProps>(({
  ...
  cooldownMs = 1200,
  onPhotoCaptured,
  onReceiptParsed: _onReceiptParsed,
  ...
  const _shouldWarnNoStore = isReceiptMode && !selectedStoreId && !receiptSaveDisabled;
  ...
  }, [acceptScan, stopScanner, validateUpc, isReceiptMode, useNativeCameraOnly, startCamera, stopCamera, mode, modeFallbackHint, streamRef, videoRef]);
  ...
  const handleReceiptFile = useCallback(
    ...
    [startScanner, addToast, stopScanner]
  );

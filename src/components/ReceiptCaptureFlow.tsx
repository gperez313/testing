...
interface ReceiptCaptureFlowProps
  ...
  onReceiptCreated?: (_captureId: string) => void;
  onParsedItems?: (_items: any) => void;
  onImageUploaded?: (_url: string, _thumbnailUrl?: string) => void;

  /** Called when the flow should close (camera exit or after capture) */
  onCancel?: () => void;

  /** Existing callbacks you already use elsewhere */
  onStoreSelected?: (_storeId: string) => void;
  onPrimarySupplierToggle?: (_storeId: string, _nextValue: boolean) => void;
}

const PRIMARY_SUPPLIER_SESSION_KEY = 'receipt.primarySupplierOverrides';

const _formatStoreBrand = (storeType?: string) =>
...
const ReceiptCaptureFlow: React.FC<ReceiptCaptureFlowProps> = ({
  ...
  onPrimarySupplierToggle: _onPrimarySupplierToggle,
  ...scannerProps
}) => {
  ...
  const [_previewImage, _setPreviewImage] = useState<string | null>(null);
